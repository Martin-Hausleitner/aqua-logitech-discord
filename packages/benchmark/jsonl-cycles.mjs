...
const INTENT=new Set(['recording','source','hookSeq','hookMonoNs','intentMonoNs']),CONF=new Set(['recording','source','confirmationMonoNs']),DISC=new Set(['muted','online','stateSeq','clientMonoMs']);
...
export function parseJsonl(input){
 ...
 return lines.filter(x=>typeof x==='string'&&x.trim()).map((raw,i)=>{let f;try{f=JSON.parse(raw)}catch{throw Error('malformed JSON at line '+(i+1))} fields(f,TOP,'top');
  ...
  if(f.intent!==null){fields(f.intent,INTENT,'intent');if(typeof f.intent.recording!=='boolean'||typeof f.intent.source!=='string'||('intentMonoNs'in f.intent&&!digits(f.intent.intentMonoNs)))throw Error('invalid intent')}
