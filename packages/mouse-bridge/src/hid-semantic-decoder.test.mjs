...
test('delta changedBytes compares against the prior report', () => {
  const d = createHidSemanticDecoder();
  assert.equal(d({ report: [0x10, 0x20] }, 0).delta, null);
  const out = d({ report: [0x11, 0x22] }, 1);
  assert.deepEqual({ ...out.delta, changedBytes: Array.from(out.delta.changedBytes) }, { from: '1020', to: '1122', changedBytes: [0x01, 0x02] });
});
