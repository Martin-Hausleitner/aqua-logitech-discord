...
if (!disc || disc.online!==true) {
  ...
  if (last?.apps?.discord?.muted === true) {
    ws.send(JSON.stringify({ type: "set_mute", mute: false }));
    for (let i=0;i<10;i++){ await new Promise(r=>setTimeout(r,200)); if (last?.apps?.discord?.muted===false) break; }
  }
  await post("/button1");
  for (let i=0;i<10;i++){ await new Promise(r=>setTimeout(r,100)); if (last?.recording) break; }
  if (!last?.recording) {
    ws.send(JSON.stringify({ type: "set_recording", recording: true }));
  ...
  await post("/button1");
  if (last?.recording) {
    ws.send(JSON.stringify({ type: "set_recording", recording: false }));
  }
